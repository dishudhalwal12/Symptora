  ...
  useEffect(() => {
    ...
    void Promise.resolve().then(async () => {
      if (isDemoId(params.id)) {
        const payload = getDemoRecordById(params.id);
        setRecord(payload);
        setAssessment(payload?.linkedAssessmentId ? getDemoAssessmentById(payload.linkedAssessmentId, { uid: payload.uid }) : null);
        setLoading(false);
        return;
      }

      try {
        const payload = await recordsService.getRecordById(params.id);
        setRecord(payload);
